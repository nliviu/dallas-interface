...
void Dallas::writeScratchPad(const uint8_t* deviceAddress, const uint8_t* scratchPad)
{
 ...
 */
bool Dallas::setResolution(const uint8_t* deviceAddress, uint8_t newResolution, bool skipGlobalBitResolutionCalculation)
{
    /*
 ...
 */
int16_t Dallas::calculateTemperature(const uint8_t* deviceAddress, uint8_t* scratchPad)
{
...
void Dallas::blockTillConversionComplete(uint8_t bitResolution)
{
    ...
    if (_checkForConversion && !_parasite) {
        ...
        while (!isConversionComplete() && ((uint64_t) (mgos_uptime()*1000 * 1000) - delms < now));
    } else {
        mgos_usleep(delms);
    }
}
